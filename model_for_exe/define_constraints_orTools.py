def define_constraints(self):
    # CP-SAT : operation/vehivle 별로 interval변수를 모아두었다가 AddCumulative로 제약 포함시킴
    operation_to_intervals_dict = {op: [] for op in self.operation_list}
    vehicle_to_intervals_dict = {v: [] for v in self.vehicle_dict.keys()}
    ...
    for (vehicle_name, operation_name), var in self.operation_var_dict_by_vehicle_operation_dict.items():
        ...
        if self.operation_dict[operation_name].order == 2:
            self.model.Add(self.starts[(vehicle_name,operation_name)] == self.operation0_dict[vehicle_name])
        # 토요일 검사 불가능
        if self.operation_dict[operation_name].test_operation:
            for saturday_index in self.calendar.saturday:
                self.model.Add(self.starts[(vehicle_name,operation_name)] != saturday_index)
        # 휴일 작업 불가능
        for holiday_index in self.calendar.holiday:
            self.model.Add(self.starts[(vehicle_name,operation_name)] != holiday_index)
        ...
        for same_sequence in self.same_sequence_constraint:
            if self.operation_list.index(operation_name) in same_sequence[1:]:
                ...
                if (vehicle_name, self.operation_list[same_sequence[0]]) in self.operation_var_dict_by_vehicle_operation_dict:
                    self.model.Add(self.starts[(vehicle_name, operation_name)] == 
                                    self.starts[(vehicle_name, self.operation_list[same_sequence[0]])])
                break
        if same_sequence_condition:
            operation_to_intervals_dict[operation_name].append(var)
            vehicle_to_intervals_dict[vehicle_name].append(var)
        ...
        for (vehicle_name2, operation_name2), var2 in self.operation_var_dict_by_vehicle_operation_dict.items():
            ...
            if (vehicle_name != vehicle_name2 and operation_name == operation_name2 and
                ...
                self.model.Add(self.starts[(vehicle_name, operation_name)] <= self.starts[(vehicle_name2, operation_name2)])
    
    # 공정 별 capacity 제약 ; intervalVar 제약 부과
    for op, intervals in operation_to_intervals_dict.items():
        if intervals:
            self.model.AddCumulative(intervals, [1] * len(intervals), 2)
    for v, intervals in vehicle_to_intervals_dict.items():
        if intervals:
            self.model.AddCumulative(intervals, [1] * len(intervals), 2)
