...
def build_schedule_from_solution(self):
    """
    save_results()에서 엑셀 만들 때와 동일한 근거(self.solution, var dict)를 사용해서
    차량별 스케줄을 메모리에서 바로 구성한다.

    return:
      vehicle_ops: dict[vehicle_name, list[operation_name]]  (날짜순)
      plan_date:   dict[vehicle_name, dict[operation_name, day_idx]]
    """
    if not self.solution:
        raise RuntimeError("No solution found. Solve the model first.")

    plan_date = {}    # vehicle -> {op_name: day_idx}
    vehicle_ops = {}  # vehicle -> [op_name, ...] sorted by day

    # (vehicle, operation_key) -> intervalVar
    for (vehicle_name, operation_key), var in self.operation_var_dict_by_vehicle_operation_dict.items():
        day_idx = int(self.vehicle_dict[vehicle_name].operation_dict[operation_key])

        # ★중요: operation_key 자체가 공정명이 아닐 수 있으니, operation_name으로 변환
        if operation_key in self.operation_dict:
            op_name = self.operation_dict[operation_key].operation_name
        else:
            # fallback (그래도 없으면 key를 문자열로)
            op_name = str(operation_key)

        plan_date.setdefault(vehicle_name, {})[op_name] = day_idx

    # 날짜순으로 정렬된 operation list 만들기
    for vehicle_name, op_to_day in plan_date.items():
        ops_sorted = [op for op, _ in sorted(op_to_day.items(), key=lambda kv: kv[1])]
        vehicle_ops[vehicle_name] = ops_sorted

    return vehicle_ops, plan_date


def resource_path(relative_path):
    """ PyInstaller의 임시 폴더 경로를 반환 (빌드 후 실행 시 작동) """
    try:
...
def plot_network(self):
    ...
    df_proc4 = df_proc[(df_proc['pcCd'] == 'PROC0004') | (df_proc['pcCd'] == 'PROC0005')].copy()
    df_proc4['pc'] = (df_proc4.reset_index(drop=True).index + 1).astype(str)
    ...
    node_colors = [node_color_for(G.nodes[n]) for n in G.nodes()]
    ...
    result_path = os.path.join(self.config['folderpath'], 'Activity_Network.png')
    ...
    vehicle_dict, plan_date = build_schedule_from_solution(self)
    ...
    for vehicle_name, operations in list(vehicle_dict.items()):
        ...
        sub_path = os.path.join(sub_dir, f"Activity_Network_of_Vehicle-{vehicle_name}.png")
