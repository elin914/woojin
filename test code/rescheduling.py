...
for name, subway_car in subway_car_dict.items():
    ...
    for activity_name, date in subway_car.activity_dict.items():
        if temp_date is None:
            ...
            if temp_date == date:
                result1.append((process_list.index(temp_name), process_list.index(activity_name)))
            elif temp_date + 1 == date:
                result2.append((process_list.index(temp_name), process_list.index(activity_name)))
            else:
                except_list.append((process_list.index(temp_name), process_list.index(activity_name)))
